...
Declare the canonical form of authentication data on the frontend client—registration forms, login forms, token responses, and error responses—with policies verified against the Bridge to prevent silent divergence from acceptance to rejection (422) in production. The frontend SHALL validate locally before calling the Bridge, combining UX (form-level email and password validation) with parity (72-byte UTF-8 ceiling for bcrypt).
