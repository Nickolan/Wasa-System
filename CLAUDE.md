...
- FSD (frontend): **NUNCA** una capa importa de una capa superior (`app → pages → widgets → features → entities → shared` es unidireccional); `shared/` no conoce nada de dominio WASA; `entities/` define tipos, schemas Zod y estado de dominio compartido entre features (p. ej. stores Zustand como `authStore`), sin lógica de UI ni de presentación. (Enmendado en CHANGE-16/feature-auth, decisión D-3: el estado de sesión es estado de dominio de la entidad `user`, no un detalle de `app/`.)
...
- **Gate manual antes de `apply` y `archive`**: por decisión explícita del usuario (2026-08-22), el orquestador tiene PROHIBIDO delegar `/opsx:apply` o `/opsx:archive` sin permiso previo del usuario en el turno actual — sin importar el nivel de governance del change (aplica incluso a LOW/MEDIUM). `/opsx:propose` (y su ida y vuelta de revisión de decisiones) sí puede correr sin pedir permiso extra. Después de cada `propose` (o de cerrar un checkpoint de decisiones), el orquestador se detiene y pregunta explícitamente si avanza a `apply`; después de un `apply` exitoso, se detiene y pregunta explícitamente si avanza a `archive`. Un "dale segui" genérico al inicio de un change (p. ej. "vamos con el change-02") NO autoriza los tres pasos completos — autoriza arrancar el flujo, no saltarse los gates de apply/archive.
