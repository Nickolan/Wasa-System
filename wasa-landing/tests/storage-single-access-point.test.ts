...
describe('localStorage: punto único de acceso (spec: "Punto único de acceso al almacenamiento", D-12)', () => {
  it('no file under src/ other than app/stores/authStore.ts mentions localStorage', () => {
    const offenders = listSourceFiles(srcRoot)
      .filter((f) => f !== 'app/stores/authStore.ts')
      .filter((f) => readFileSync(path.join(srcRoot, f), 'utf-8').includes('localStorage'))

    expect(offenders).toEqual([])
  })

  it('app/stores/authStore.ts is the sole module invoking hydrate (spec: "Punto único de restauración")', () => {
    const offenders = listSourceFiles(srcRoot)
      .filter((f) => f !== 'app/stores/authStore.ts')
